...
def send_text(request, *args, **kwargs):
	...
	if request.POST.get('content', None):
		...
		if os.path.isfile('%savatar/%s.jpg' % (settings.MEDIA_ROOT, request.POST['uid'])):
			avatar = '%s_s.jpg' % (request.POST['uid'])
		else:
			avatar = 'default_s.jpg'
		cursor = connection.cursor()
		cursor.execute('select id from Magicword order by id desc limit 1')
