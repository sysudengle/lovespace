...
#		msg = Message(userId_id = long(info['uid']),
		...
		cursor.execute('select Message.id, Message.image, User.avatar from Message, User where Message.userId=User.id order by Message.id desc limit 1')
		...
		data = {'user_name': info['user'],
				...
				'lid': request.session['lid'], 'avatar': row[0][2],
				'uid': info['uid']}
		return render_to_response('message_basic.html', {'single_message': data}, context_instance = RequestContext(request))
...
def get_comment(request, *args, **kwargs):
	if request.method == 'POST':
		data = []
		cursor = connection.cursor()
		cursor.execute('select User.name, User.id, Comment.content, Comment.time, Comment.messageId, Comment.id, User.avatar from User, Comment where User.id=Comment.userId and Comment.messageId=' + request.POST['mid'] + " order by Comment.id desc")
		rows = cursor.fetchall()
		for row in rows:
			data.append({"all": rows[0], "user_name": row[0], "user_id": row[1], "content": row[2], "time": row[3], "mid": row[4], "cid": row[5], "avatar": row[6]})
...
def send_comment(request, *args, **kwargs):
	if request.method == 'POST' and request.POST.get('uid', None) and request.POST.get('content', None):
		...
		p.save()
		cursor = connection.cursor()
		cursor.execute('select Comment.id, User.avatar from User, Comment where Comment.userId=User.id order by Comment.id desc limit 1')
		row = cursor.fetchall()	# row[0][0] represent message id
