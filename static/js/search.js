...
//		urlRoot: '/lovers/add_concern/',
	...
	{
		$('.add_request').click(add_request);
		$('.delete_request').click(delete_request);
	}
	
	function add_request(e)
	{
		var btn = $(e.currentTarget);
		var tid = btn.data('uid');
		var fid = info.uid;
		var des_url = '/' + fid + '/user/add_request/' + tid + '/';
		$.ajax({
			type: 'POST',
			url: des_url,
			data: {'fid': fid, 'tid': tid},
			success: function(result){
				btn.attr('class', 'delete_request btn btn-danger');
				$('.add_request').attr('class', 'ban btn disabled');
				$('.delete_request').html('取消邀请');
				$('.delete_request').click(delete_request);
			},
			error: function(result) {
				//alert('fail~add');
	...
	function delete_request(e)
	{
		...
		var tid = btn.data('uid');
		var fid = info.uid;
		var des_url = '/' + fid + '/user/delete_request/' + tid + '/';
		$.ajax({
			type: 'POST',
			url: des_url,
			data: {'fid': fid, 'tid': tid},
			success: function(result){
				btn.attr('class', 'add_request btn btn-primary');
				$('.ban').attr('class', 'add_request btn btn-primary');
				btn.html('邀请伴侣');
				$('.add_request').click(add_request);
			},
			error: function(result) {
